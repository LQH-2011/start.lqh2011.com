...
module.exports = async function handler(req, res) {
  ...
  if (!password) {
    lib.send(res, 400, { error: 'missing_password' }, req);
    return;
  }

  if (lib.verifyPassword(password)) {
    lib.clearRateLimit(req);
    lib.send(res, 200, { token: lib.signToken() }, req);
    return;
  }
  ...
  if (lib.recordFailure(req)) {
    lib.send(res, 429, { error: 'too_many_attempts' }, req);
    return;
  }
  lib.send(res, 401, { error: 'invalid_password' }, req);
};
