...
- **Command mode**: type `aaa` (or `Aaa` / `/`, case-insensitive) in the search bar to
  enter command mode (the magnifier icon is replaced by a `❯` indicator on the left;
  the input stays left-aligned). Then (letters are case-insensitive):
